...
from tkinter import Tk, Label, Button, LabelFrame, Canvas, PhotoImage, NW, messagebox, Menu, Toplevel, ttk,font
...
class UI(object):
    def __init__(self, master):
        ...
        self.details_frame = LabelFrame(master, text="Game Action : ")
    ...
    def update_detail_labels(self):
        ...
        self.goats_in_hand_label.configure(text=f"Goats Remain: {self.game.goats_in_hand}")
    ...
    def board_frame_init(self):
        self.canv = Canvas(self.board_frame, width=700, height=700, bg='#CDC673')
        self.canv.grid(row=0)

    def settings(self):
        setting_window = Toplevel(self.master)
        setting_window.title("Settings")
        setting_window.geometry("250x150")
        setting_window.resizable(False,False)
        setting_window.transient(self.master)
        setting_window.grab_set()

        goat_label = Label(setting_window, text="Goat Role")
        ...
        tiger_label = Label(setting_window, text="Tiger Role")
        ...
        okay = Button(setting_window, text="Save", command=lambda:self.save_settings(setting_window,options_1, options_2, '''difficulty_options'''))
        okay.grid(row=3, column=0)
    
    
        

    def save_settings(self,setting_window,options_1, options_2, difficulty_options):
        parser["settings"]["goat"] = options_1.get()
        parser["settings"]["tiger"] = options_2.get()
        # parser["settings"]["difficulty"] = difficulty_options.get()

        goat_role = options_1.get()
        tiger_role = options_2.get()
        if goat_role == tiger_role == "AI":
            messagebox.showinfo("Alert", "Both Goat and Tiger cannot be set to AI. Please reselect the options.")
        else:
         with open(self.assets.configuration_path, "w") as f:
            
             parser.write(f)
    
         messagebox.showinfo("Settings saved. ", "Settings updated. click Newgame to apply the changes") 
    ...
    def menu_init(self):
        ...
        menubar.add_command(label="Quit!", command=lambda: self.quit_game())
        
        # display the menu
        self.master.config(menu=menubar)

    def quit_game(self):
        if messagebox.askyesno("Quit Baaghchal", "Are you sure you want to quit the game?"):
            self.master.quit()
    ...
    def ui_init(self):
        ...
        custom_font = font.Font(weight="bold",size=14)
        self.master.title("Baagchal with AI")
        self.master.geometry("950x800")
        self.master.resizable(False, False)

        Label(self.master, text="Baagchal",font=custom_font).grid(row=0, column=0)
